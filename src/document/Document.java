...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (int i = 0; i < cArray.length; i++) {
			if (i == cArray.length - 1 && Character.toLowerCase(cArray[i]) == 'e' && newSyllable && SyllablesCount > 0) {
				SyllablesCount--;
			}
	...
	{
		double numberOfWords = (double) getNumWords() ;
		double wordratio = ((double)numberOfWords / (double) getNumSentences()) ;
		double syllableratio = (double)  ((double)getNumSyllables()/(double)numberOfWords);
		double result = 206.835 -((1.015 * wordratio) - (84.6*syllableratio )) ;
	    return result ;
	}
